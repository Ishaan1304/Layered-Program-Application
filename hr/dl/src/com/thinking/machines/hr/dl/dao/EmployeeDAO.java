...
throw new DAOException("PANNumber ("+panNumber+") and Aadhar card number ("+aadharCardNumber+") exists.");
}
...
if(employeeId.length()==0) throw new DAOException("Invalid employee Id.: employe is of zero length");
try
{
File file=new File(FILE_NAME);
if(file.exists()==false) throw new DAOException("Invalid employee Id.:"+employeeId);
...
panNumber=panNumber.trim();
if(panNumber.length()==0) throw new DAOException("Invalid PAN number: PAN is of zero length");
try
{
File file=new File(FILE_NAME);
...
{
randomAccessFile.close();
throw new DAOException("Invalid Aaddhar card number:"+aadharCardNumber);
}
randomAccessFile.readLine();
randomAccessFile.readLine();
EmployeeDTOInterface employeeDTO;
